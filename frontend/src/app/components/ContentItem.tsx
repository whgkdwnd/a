...
export function ContentItem({
  ...
  return (
    ...
    >
      ...
      {progress != null && (
        <div className={`text-sm font-bold ${completed ? 'text-gray-400' : 'text-gray-700'}`}>{progress}</div>
      )}
      {showArrow && (
        <button type="button" className="p-1.5 rounded-full bg-gray-400 hover:bg-gray-500 transition-colors">
          <ChevronRight className="w-4 h-4 text-white" />
        </button>
      )}
      {!showArrow && onToggleComplete != null && (
        <button
          type="button"
          onClick={(e) => {
            e.stopPropagation()
